...
def get_youtube_video_urls(query, num_results=10):
    try:
...
# Add a "Refresh" button to allow users to refresh the song list based on the new city
refresh_button = st.button("Search & Refresh Song Suggestions")

# When the button is clicked or city is provided, get weather data and song suggestions
if city or refresh_button:
    if city:
        # If city is provided, get the weather data
        ...
        if weather_data.get("cod") != 200:  # In case of error (invalid city)
            ...
            # Fetch new video URLs based on the weather query
            st.session_state.video_urls = get_youtube_video_urls(query, num_results=10)  # Fetch 10 videos
